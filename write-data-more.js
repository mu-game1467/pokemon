...
const content = [
  ...
  "  const teams = loadJs(path.join(DATA_DIR, 'pokedb-teams.js'));",
  "",
  "  test('has seasons with teams', () => {",
  "    const keys = Object.keys(teams);",
  "    expect(keys.length).toBeGreaterThan(0);",
  "    const some = teams[keys[0]];",
  "    if (Array.isArray(some)) {",
  "      expect(some.length).toBeGreaterThan(0);",
  "    } else {",
  "      expect(Object.keys(some)).not.toHaveLength(0);",
  "    }",
  "  });",
  "",
  "  test('team structure is consistent', () => {",
  "    const seasons = Array.isArray(teams)",
  "      ? teams",
  "      : Object.values(teams).flatMap(v => Array.isArray(v) ? v : Object.values(v));",
  "    const teamsList = seasons.filter(t => Array.isArray(t) && t.length > 0);",
  ...
  "        expect(member).toHaveProperty('rating');",
  "      });",
  "    });",
  "  });",
  "});",
...
const out = path.join(__dirname, 'tests', 'unit', 'data-more.test.js');
fs.writeFileSync(out, content, 'utf8');
console.log('WROTE ' + out + ' len=' + content.length);
